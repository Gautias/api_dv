...
import CardException from '../exception/CardException';
...
export const subMid = (req: Request, res: Response, next: () => void) => {

    let data: any = req.body;
    const champsRequire = [`card_number`, `cvc`]
    try{
        let error: boolean = true;
        let textError: string = '';
        for (const require in champsRequire) {
        ...
        // if (CardException.checkCard(data.card_number)) // Check valid card number
        //     console.log(CardException.checkCard(data.card_number));
        //     throw new CardException();

        // if (CardException.checkCvc(data.cvc))
        //     console.log(CardException.checkCvc(data.cvc));
        //     throw new CardException();
